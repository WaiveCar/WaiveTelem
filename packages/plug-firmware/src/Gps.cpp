...
void GpsClass::setup() {
  ...
  lastSentTime = -10000 * 1000;  // small enough to make sure one message get sent out immediately
}
  ...
  while (gps.available(GPSSerial)) {
    ...
    if (fix.valid.location /* && fix.valid.date && fix.valid.time */) {
      ...
      if (interval >= 0 && millis() - lastSentTime > (u_int32_t)interval) {
        ...
        Mqtt.telemeter("{" + Status.getStatus() + ", \"gps\": {\"lat\": " + String(latitude) + ", \"long\": " + longitude + /* ", \"time\": \"" + time + "\"" + */ "}}");
        lastSentTime = millis();
      }
    }
  }
